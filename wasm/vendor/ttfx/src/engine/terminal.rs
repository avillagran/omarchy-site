...
pub struct TerminalConfig {
    ...
    /// Preserve top-left input coordinates for embedded full-canvas renderers.
    pub preserve_input_layout: bool,
...
impl Terminal {
    pub fn new(input_data: &str, config: TerminalConfig) -> Result<Self, EngineError> {
        ...
        let mut terminal = Terminal {
            ...
            move_cursor_to_top,
            frame_rate,
            // The browser bridge drives a virtual clock and frame pacing in
            // requestAnimationFrame; wasm32-unknown-unknown has no native
            // monotonic clock for std::time::Instant.
            last_time_printed: None,
        };
    ...
    pub fn enforce_framerate(&mut self) {
        ...
        let Some(last_time_printed) = self.last_time_printed else { return; };
        let elapsed = last_time_printed.elapsed().as_secs_f64();
...
fn setup_input_characters(
    ...
    let input_height = if config.preserve_input_layout { canvas.top } else { formatted_lines.len() as i64 };
    ...
    canvas
        .anchor_text(arena, input_characters, if config.preserve_input_layout { Anchor::Sw } else { config.anchor_text })
